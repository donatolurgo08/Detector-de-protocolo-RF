...
void mostrarEspera() {
  ...
  display.print("315 y 433 MHz activo");
...
void setup() {
  ...
  sw433.enableReceive(0); // D2
  sw315.enableReceive(1); // D3
...
bitsDetectados = sw433.getReceivedBitlength();
    ...
    if (freqDetectada == "")
      freqDetectada = "433.92 MHz";

    codigos[indice % 3] = cod;
    indice++;

    Serial.print("433MHz | Cod:");
    Serial.print(cod);
...
bitsDetectados = sw315.getReceivedBitlength();
    ...
    if (freqDetectada == "")
      freqDetectada = "315 MHz";

    codigos[indice % 3] = cod;
    indice++;

    Serial.print("315MHz | Cod:");
    Serial.print(cod);
